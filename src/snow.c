...
void SNOW_draw(void)
{
    ...
    /* we only draw if we are on and haven't toggled (see SNOW_Erase for more
     * info */
    if (SNOW_on != 1)
    {
        return;
    ...
    {
        to = screen->pixels;
        to += (SNOW_rects[NUM_FLAKES + i].y << 9) +
              (SNOW_rects[NUM_FLAKES + i].y << 7) +
              SNOW_rects[NUM_FLAKES + i].x;
        *to = snow_color;
        to += 1;
        *to = snow_color;
        to += 640;
...
void SNOW_erase(void)
{
    ...
    for (i = 0; i < NUM_FLAKES; i++)
    {
        ...
        from +=
            (SNOW_rects[i].y << 9) + (SNOW_rects[i].y << 7) + SNOW_rects[i].x;
        *to = *from;
        to += 1;
        from += 1;
        *to = *from;
